...
{
    BYTE originalRed, originalBlue, originalGreen, avg;
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            avg = round((originalRed + originalBlue + originalGreen) / 3.0);
...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            int sumRedGx = 0, sumRedGy = 0;
            int sumGreenGx = 0, sumGreenGy = 0;
            int sumBlueGx = 0, sumBlueGy = 0;

            // All Pixels Around the selected One

            if ((j + 1) < width) // Right pixel
            {
                sumRedGx += (2 * image[i][j + 1].rgbtRed);
                sumGreenGx += (2 * image[i][j + 1].rgbtGreen);
                sumBlueGx += (2 * image[i][j + 1].rgbtBlue);
            }
            if ((j - 1) >= 0) // Left pixel
            {
                sumRedGx += (-2 * image[i][j - 1].rgbtRed);
            ...
            if ((i - 1) >= 0) // Upper pixel
            {
                sumRedGy += (-2 * image[i - 1][j].rgbtRed);
                sumGreenGy += (-2 * image[i - 1][j].rgbtGreen);
                sumBlueGy += (-2 * image[i - 1][j].rgbtBlue);
            }
            if ((i + 1) < height) // Lower pixel
            {
                sumRedGy += (2 * image[i + 1][j].rgbtRed);
            ...
            if ((i - 1) >= 0 && (j + 1) < width) // Upper Right pixel
            {
                sumRedGx += (1 * image[i - 1][j + 1].rgbtRed);
                sumGreenGx += (1 * image[i - 1][j + 1].rgbtGreen);
                sumBlueGx += (1 * image[i - 1][j + 1].rgbtBlue);

                sumRedGy += (-1 * image[i - 1][j + 1].rgbtRed);
                sumGreenGy += (-1 * image[i - 1][j + 1].rgbtGreen);
                sumBlueGy += (-1 * image[i - 1][j + 1].rgbtBlue);
            }
            if ((i - 1) >= 0 && (j - 1) >= 0) // Upper Left pixel
            {
                sumRedGx += (-1 * image[i - 1][j - 1].rgbtRed);
            ...
            int edgesRed = round(sqrt(pow(sumRedGx, 2) + pow(sumRedGy, 2)));
            int edgesBlue = round(sqrt(pow(sumBlueGx, 2) + pow(sumBlueGy, 2)));
            int edgesGreen = round(sqrt(pow(sumGreenGx, 2) + pow(sumGreenGy, 2)));

            edgesRed = (edgesRed > 255) ? max : (BYTE) edgesRed;
            edgesGreen = (edgesGreen > 255) ? max : (BYTE) edgesGreen;
            edgesBlue = (edgesBlue > 255) ? max : (BYTE) edgesBlue;

            image2[i][j].rgbtRed = edgesRed;
            image2[i][j].rgbtGreen = edgesGreen;
            image2[i][j].rgbtBlue = edgesBlue;
        }
    }
